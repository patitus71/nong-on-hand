...
//
// Vercel cron job: ส่ง EOD summary ไปยัง squad ที่ตั้ง eodAutoSendEnabled=true
// และ eodSendTime ตรงกับเวลา ICT ปัจจุบัน (HH:MM)
//
...
export async function GET(req: Request) {
  ...
  for (const [groupId, groupSquads] of Array.from(byGroup)) {
    ...
    console.log(`[cron/eod] time=${nowICT} group=${groupId} squads=${groupSquads.map(s => s.name).join(',')} chunks=${chunks.length}`);
